...
int main(int argc, char **argv)
{
  ...
  FILE *dmpfile;
  FILE *dmpfile_h;
  char outname_dmp[256];
  char outname_h  [256];
  unsigned long int m64_tt_byte_ctr = 0;
  ...
  if ((argc < 2) || (usage))
  {
    printf("Usage: SIDDUMP <sidfile> [options]\n"
           ...
           "\nM64: siddmp still writes its outpout to stdout, additionaly it creates:\n"
           "<sidfile>.dmp   - a binary sid dump of the first 25 bytes at $d400, each frame\n" 
           "<sidfile>_dmp.h - a C header file of the binary ready to include\n\n" 
           "These files can be used for playback on microcontrollers or\n"
           "by any siddmp (playback) utility for further investigation.\n"
           "Especially useful to debug hardrestart/instrument timings.\n"
           );
    return 1;
  }
  ...
  outname_dmp[0] = 0x00;
  outname_h[0] = 0x00;
  strncpy(outname_dmp, sidname, 255);
  strncat(outname_dmp, ".dmp", 255);
  strncpy(outname_h, sidname, 255);
  strncat(outname_h, "_dmp.h", 255);

  printf("Writing DUMP files to: '%s' and '%s'.\n", outname_dmp, outname_h);
  dmpfile = fopen(outname_dmp, "w");
  dmpfile_h = fopen(outname_h, "w");
  if(!dmpfile || !dmpfile_h) {
    printf("Error: couldn't open DMP files.\n");
    return 1;
  }
  ...
  fprintf(dmpfile_h, "unsigned char siddmp[] = {\n");
  while (frames < firstframe + seconds*50)
  {
    ...
    if (frames >= firstframe)
    {
      ...
      // m64 
      // write to dmp files
      for(int linebytes=0; linebytes<25; linebytes++) {
        if(!linebytes) fprintf(dmpfile_h,"\t");
        fprintf(dmpfile_h, "0x%02x",mem[0xd400 + linebytes]);
        fprintf(dmpfile,       "%c",mem[0xd400 + linebytes]);
        if(linebytes != 24) fprintf(dmpfile_h, ",");
        else { 
          if(frames < firstframe + seconds*50-1) fprintf(dmpfile_h, ",\n"); 
          else fprintf(dmpfile_h, "\n");
        }
        m64_tt_byte_ctr++;
      }
  ...
  fprintf(dmpfile_h, "};\n\n" );
  fprintf(dmpfile_h, "unsigned int siddmp_len=%lu;\n", m64_tt_byte_ctr);
